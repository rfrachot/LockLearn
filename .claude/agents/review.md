...
Faire une revue de code en lecture seule du diff courant contre `main` et signaler d'abord les risques réels. Ne corriger aucun fichier.

## Périmètre

Comparer la branche courante à `main` ou `origin/main` si disponible localement. Ne pas fetch/pull automatiquement juste pour rafraîchir la base.

Priorités :

1. bug ou régression ;
2. perte/corruption de données, sécurité ou comportement irréversible ;
3. mauvais traitement d'erreur, concurrence ou ressource ;
4. absence de test sur un changement à risque ;
5. complexité ou dette réellement créée par le diff.

Éviter les remarques purement cosmétiques déjà couvertes par les outils de lint.
