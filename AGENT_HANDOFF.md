...
The repeated-failure blueprint fires only when `consecutive_wrong` equals the
configured threshold, avoiding repeated encouragement on every later failure.
Its optional Track pause deliberately re-enters P4.8 unattended authorization;
...
handoff, learning/quiz/relearning channels, private-by-default visibility,
shared-device Profile labeling, stable target delivery with dynamic mobile_app
route resolution, target-unavailable Repairs, and a private dashboard warning
for expired mobile action attempts that could not be recorded.

The direct-exposure renderer is explicitly `exposure_only` and its test passes
that result through SignalPolicy to prove it cannot promote a review box.
Profile/target identities remain distinct throughout rendered payload and
delivery. No DB migration was required.

Final P4.7 qualification on 2026-09-24: resource registries PASS and pytest PASS
(367 tests in 18.01 s). Ruff reported only three mechanical formatting
differences plus four fixable style/import findings; mypy reported one test-only
optional `action_semantics` assertion. Those exact mechanical/type-test fixes
were applied afterward with no semantic runtime change.

P4.8 owns HA action/result services and pedagogical events. It must decode the
content-free action ID, consume the P4.6 token, and only then apply any
ReviewEvent/Progress mutation.

Next concrete action: begin P4.8 HA services/actions and pedagogical events.
...
No DB migration was required. P4.7 remains responsible for renderers/action
...
Final P4.6 qualification on 2026-09-24: Ruff lint PASS, mypy PASS (128 sources),
resource registries PASS and pytest PASS (357 tests in 17.16 s). Ruff format
initially reported only three mechanical formatting diffs in the new P4.6 files;
those exact formatting changes were applied afterward with no semantic change.

Next concrete action: begin P4.7 notification renderers and privacy-safe target
behavior.
...
selection reason are persisted on the slot; rendered learned text is not copied
into `state.db`, and retries reuse the same binding.

General ranking is relearning due → review due → due leech/difficult recoverable
→ calibration-needed → bounded new teaser. Future reviews are not pulled forward
just to fill a slot. New teasers are learning-only and limited to two per
Profile-local day. Pre-sleep routine slots restrict to today's introductions;
morning-first-review restricts to yesterday's introductions.
...
Backoff changes only notification capacity, does not advance the P4.3 capacity
Repair streak, and never mutates Progress/ReviewEvent.
...
ADR-0040 is accepted.

Final P4.5 qualification on 2026-09-24: Ruff format PASS (225 files), Ruff lint
PASS, mypy PASS (126 sources), resource registries PASS and pytest PASS
(350 tests; final full functional run 16.83 s).

Next concrete action: begin P4.6 persistent notification interactions and replay
protection. P4.7+ rendering, services/actions and blueprints remain out of scope
until their dedicated work packages.
...
can attach the first delivery-to-action latency. These samples remain
observational only and V1 does not auto-learn preferred schedule hours.
...
only to configured entities, runs its async state-change handler on the HA event
loop, refreshes after Profile CRUD and detaches on unload. Routine slots remain
content-free and still respect P4.3 Profile/target/device capacity plus
active-session suppression. ADR-0039 is accepted.

Final P4.4 development-checkout gate on 2026-09-24: Ruff format PASS (222 files),
Ruff lint PASS, mypy PASS (124 sources), resource registries PASS and pytest PASS
(340 tests in 16.59 s).

Next concrete action: begin P4.5 notification selection policy,
missed/pending/backoff. P4.6+ interaction replay protection and notification
rendering remain out of scope until their dedicated work packages.
...
expose both real UTC instants while sharing one local-hour capacity bucket, and
...
rewriting sent/consumed history. Profile timezone changes create a new scheduler
config version, cancel only older future unsent rows and generate in the new
timezone. No state schema migration was required.
...
real Config Entry reload persistence and timezone changes. ADR-0037 is accepted.
Final P4.2 development-checkout gate on 2026-09-24: Ruff format PASS (219 files),
Ruff lint PASS, mypy PASS (123 sources), resource registries PASS and pytest PASS
(328 tests in 15.51 s). The scheduler still does not use theoretical slot time
for SRS evidence; real retrieval/response timestamps remain the interval source.

Next concrete action: begin P4.3 multi-track/multi-target arbitration and
capacity Repairs. P4.4+ receptivity, missed/pending/backoff and notification
interaction behavior remain out of scope until their dedicated work packages.
